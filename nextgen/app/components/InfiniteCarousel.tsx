...
export default function InfiniteCarousel({ items }: InfiniteCarouselProps) {
    ...
    // Config
    const GAP = 24;
    const AUTO_DELAY = 2000; // 2 Seconds
    ...
    useEffect(() => {
        const handleResize = () => {
            const newImgWidth = window.innerWidth < 768 ? 350 : 600;
    ...
    return (
        <div
            className="relative w-full h-[500px] flex flex-col justify-center overflow-hidden py-4"
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
            ref={containerRef}
        >
            {/* Gradient Masks */}
            <div className="absolute left-0 top-0 bottom-0 w-24 md:w-32 z-10 bg-gradient-to-r from-black via-black/80 to-transparent pointer-events-none" />
            <div className="absolute right-0 top-0 bottom-0 w-24 md:w-32 z-10 bg-gradient-to-l from-black via-black/80 to-transparent pointer-events-none" />
            ...
            <button
                onClick={prevSlide}
                className="absolute left-4 md:left-10 z-30 w-12 h-12 rounded-full border border-[#4DBC1B] flex items-center justify-center bg-black/50 backdrop-blur-sm text-[#4DBC1B] hover:bg-[#4DBC1B] hover:text-black hover:scale-110 transition-all duration-300 shadow-[0_0_10px_rgba(77,188,27,0.2)] group"
            >
                <ChevronLeft className="w-6 h-6" />
            </button>
            <button
                onClick={nextSlide}
                className="absolute right-4 md:right-10 z-30 w-12 h-12 rounded-full border border-[#4DBC1B] flex items-center justify-center bg-black/50 backdrop-blur-sm text-[#4DBC1B] hover:bg-[#4DBC1B] hover:text-black hover:scale-110 transition-all duration-300 shadow-[0_0_10px_rgba(77,188,27,0.2)] group"
            >
...
function CarouselCard({ item, width, isCenter }: { item: CarouselItem, width: number, isCenter: boolean }) {
    ...
    return (
        ...
        >
            ...
            {/* Content Container */}
            <div className="absolute inset-0 p-8 flex flex-col justify-end group-hover:justify-center group-hover:items-center transition-all duration-500">
                <div className="w-full transform transition-all duration-500 group-hover:-translate-y-2 group-hover:text-center">
                    <p className="text-[#4DBC1B] text-sm font-bold tracking-widest uppercase mb-2 opacity-80 group-hover:opacity-100">
                        {item.subtitle}
                    </p>
                    <h3 className="text-2xl md:text-3xl font-black text-white mb-4 shadow-black drop-shadow-lg leading-tight">
                        {item.title}
                    </h3>
                    ...
                    <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-500 ease-out">
                        <div className="overflow-hidden">
                            <p className="text-gray-200 text-sm md:text-lg opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100 leading-relaxed max-w-2xl mx-auto">
                                {item.desc}
                            </p>
                        </div>
                    </div>
