...
export default function TeamMemberCard({ name, role, image, bio, socials, delay = 0 }: TeamMemberProps) {
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            className="group relative bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 overflow-hidden hover:border-[#4DBC1B]/50 transition-colors duration-300"
        >
            {/* Glow Effect */}
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-bold text-white mb-1 group-hover:text-[#4DBC1B] transition-colors">
                    {name}
                </h3>
                <p className="text-sm text-[#4DBC1B] mb-4 font-medium tracking-wide uppercase">
                    {role}
                </p>
